...
{
    NSString *samplePath =
    [[NSBundle mainBundle] pathForResource:@"background-music.caf"
                                    ofType:nil];
    NSURL *file =
    [NSURL fileURLWithPath:samplePath];
...
- (void)playCoinCollectedSoundEffect
{
    NSString *samplePath =
    [[NSBundle mainBundle] pathForResource:@"coin-collected-sound.caf"
                                    ofType:nil];
    NSURL *file =
    [NSURL fileURLWithPath:samplePath];
...
- (void)addSpaceShipThrust
{
    NSString *emitterPath =
    [[NSBundle mainBundle] pathForResource:@"FireParticle"
                                    ofType:@"sks"];
    
    SKEmitterNode *emitterNode =
    [NSKeyedUnarchiver unarchiveObjectWithFile:emitterPath];
    
    //do some view specific tweaks
    
    emitterNode.particlePosition =
    CGPointMake(-self.spaceShipSprite.frame.size.width/2,
                0);
...
- (void)addCoinCollisionEffectWithSpaceShip
{
    NSString *emitterPath =
    [[NSBundle mainBundle] pathForResource:@"SmokeParticle"
                                    ofType:@"sks"];
...
- (void)spaceShipCollidedWithCoin:(SKNode*)coinNode
{
    ...
    [self playCoinCollectedSoundEffect];
}
